...
drop view if exists StudentMajorInfo

go
create view dbo.StudentMajorInfo with schemabinding
as
select concat(stu.LastName, ',', ' ', stu.FirstName) [Student Name]
	, maj.MajorName
	, concat(adv.LastName, ',', ' ', adv.FirstName) [Advisor Name]
from dbo.Students stu inner join dbo.Majors maj on stu.MajorID = maj.MajorID
	inner join dbo.Advisors adv on stu.AdvisorID = adv.AdvisorID

go
Select *
from StudentMajorInfo